...
export const element = {
  ...
  heights(ctx) { for (const { i, h } of ctx.marks.kicks || []) placeKicker(ctx.H0, ctx.jump0, i, h, j => ctx.nb(i, j - i)); },
  track(ctx, out) { out.kicks = (ctx.marks.kicks || []).map(k => ({ ...k, i: ctx.u0(k.i) })); },
