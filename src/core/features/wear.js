...
// until a water splash washes it off. Deterministic, no randomness.
export const WEAR = { COLS: 28, CELL: 0.5, TRACK: 0.95, DIG: { mud: 0.2, gravel: 0.035, tarmac: 0.006 }, SPREAD: 0.35, TUG: 4.5, SCRUB: 0.4, CARRY: 30, TRAIL: 0.5 };
...
const colOf = lat => Math.floor(lat / WEAR.CELL + WEAR.COLS / 2);
...
function step(R, W, dt) {
  ...
  for (const c of R.cars) {
    ...
    for (const side of [-1, 1]) {
      ...
      if (carry && c.surface !== 'mud') m[q] = Math.min(1, m[q] + WEAR.TRAIL * carry * sp * dt);
    }
...
export const feature = {
  name: 'wear',
  init(R, W) { const n = W.tr.NB * WEAR.COLS; W.wear = { g: new Float32Array(n), m: new Float32Array(n), ver: 0 }; R.wear = W.wear; for (const c of R.cars) c.mudCarry = 0; },
