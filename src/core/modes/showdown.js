// Showdown: Micro Machines-style head-to-head. The camera frames the leader; a car that drops off the screen
// loses the round and the leader takes one of its lights. Everyone then regroups at the leader's position.
...
export const SD = { START: 4, WIN: 8, OFF_SLACK: 1.5, OFF_TIME: 0.25, ANNOUNCE: 1.0, HOLD: 1.5, GRACE: 1.0, LOOK: 0.42 };
const GRID = [[0, -2.8], [0, 2.8], [-8, -2.8], [-8, 2.8]];

export function initShowdown(R) {
  R.sd = { lights: R.cars.map(() => SD.START), phase: 'run', timer: 0, grace: SD.GRACE, focus: null, snap: true, camSnap: true, offT: R.cars.map(() => 0), winner: -1, rounds: 0 };
  if (!R.view) R.view = { hw: 40, hh: 22 };
}
const active = R => R.cars.filter(c => !c.out);
export function sdLeader(R) { let best = null; for (const c of R.cars) if (!c.out && (!best || c.progress > best.progress)) best = c; return best; }

function place(c, W, i, lat) {
  const tr = W.tr;
  c.x = tr.xs[i] + tr.rx[i] * lat; c.z = tr.zs[i] + tr.rz[i] * lat; c.y = tr.H[i]; c.yaw = tr.th[i];
  c.vx = c.vz = c.vy = 0; c.vf = c.vr = 0; c.onGround = true; c.airT = 0; c.boost = 0; c.driftT = 0; c.spin = 0;
  ...
  c.ghost = SD.GRACE; c.hold = SD.HOLD;
}
/** Put every car still in the match on a 2x2 grid at the leader's position, in running order. */
...
export function showdownStep(R, W, dt) {
  ...
  if (S.phase === 'announce') { S.timer -= dt; if (S.timer <= 0) { regroup(R, W); S.phase = 'hold'; S.timer = SD.HOLD; R.player.events.push({ t: 'sd-hold' }); } return; }
  if (S.phase === 'hold') { S.timer -= dt; if (S.timer <= 0) { S.phase = 'run'; S.grace = SD.GRACE; R.player.events.push({ t: 'sd-go' }); } return; }

  S.grace -= dt; if (S.grace > 0) return;
  const hw = R.view.hw + SD.OFF_SLACK, hh = R.view.hh + SD.OFF_SLACK, losers = [];
  R.cars.forEach((c, k) => {
    if (c.out || c === L) { S.offT[k] = 0; return; }
    const [sx, sy] = screenOffset(c.x, c.y, c.z, S.focus);
    S.offT[k] = Math.abs(sx) > hw || Math.abs(sy) > hh ? S.offT[k] + dt : 0;
    if (S.offT[k] >= SD.OFF_TIME) losers.push(k);
  });
  if (!losers.length) return;
  const li = R.cars.indexOf(L);
  S.lights[li] += losers.length; for (const k of losers) S.lights[k] -= 1;
  S.rounds++; S.offT.fill(0);
  L.events.push({ t: 'sd-round', winner: li, losers });
  for (const k of losers) if (S.lights[k] <= 0) { const c = R.cars[k]; c.out = true; c.ghost = Infinity; c.events.push({ t: 'sd-out' }); }
  const left = active(R);
  if (S.lights[li] >= SD.WIN || left.length <= 1 || R.player.out) { finish(R, S.lights[li] >= SD.WIN || left.length <= 1 ? L : mostLights(R)); return; }
  S.phase = 'announce'; S.timer = SD.ANNOUNCE;
}
