...
export function initSkids() {
  ...
  skMesh = new THREE.Mesh(g, new THREE.MeshBasicMaterial({ color: 0x1A1A1A, transparent: true, opacity: 0.26, depthWrite: false, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: -2 }));
...
export function addSkid(a, b) {
  const dx = b[0] - a[0], dz = b[2] - a[2], L = Math.hypot(dx, dz) || 1, wx = -dz / L * 0.14, wz = dx / L * 0.14;   // a tyre-wide streak
