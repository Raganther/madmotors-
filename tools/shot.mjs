...
const vehicle = opt('vehicle', 'coupe'), W = +opt('w', 1100), H = +opt('h', 620), debug = flag('debug');
...
for (const m of pts) {
  const got = await page.evaluate(tgt => {
    ...
    if (d.G.world.W.wear) d.G.world.W.wear.ver++; d.step(0.2); return Math.round(P.progress - d.G.world.tr.startIdx);
  }, info.s0 + m);
